...
function ChartsGroupedXAxisTicks(inProps) {
  ...
  if (!(0, _scaleGuards.isOrdinalScale)(xScale)) {
    throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Charts: ChartsGroupedXAxis only supports the `band` and `point` scale types. ' + 'Grouped axis ticks require an ordinal scale to display category groupings. ' + 'Use a band or point scale type for the x-axis, or use a non-grouped axis component.' : (0, _formatErrorMessage2.default)(8));
  }
  const {
    disableTicks,
    tickSize,
  ...
  const autoSizeResults = store.use(_useChartAxisAutoSize.selectorChartXAxisAutoSizeResults);
