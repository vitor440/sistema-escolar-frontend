...
exports.computeOccurrencesMaxIndex = computeOccurrencesMaxIndex;
exports.useEventOccurrencesWithTimelinePosition = useEventOccurrencesWithTimelinePosition;
var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/extends"));
var React = _interopRequireWildcard(require("react"));
var _sortEventOccurrences = require("../sort-event-occurrences");
var _useAdapterContext = require("../use-adapter-context");
/**
 * Places event occurrences for a timeline UI.
 */
function useEventOccurrencesWithTimelinePosition(parameters) {
  const {
    occurrences,
    maxSpan
  } = parameters;
  ...
  return React.useMemo(() => {
    const sortedOccurrences = (0, _sortEventOccurrences.sortEventOccurrences)(occurrences);
    const conflicts = buildOccurrenceConflicts(adapter, sortedOccurrences);
    const {
      firstIndexLookup,
      maxIndex
    } = buildFirstIndexLookup(conflicts);
    const lastIndexLookup = buildLastIndexLookup(conflicts, firstIndexLookup, maxIndex, maxSpan);
    const occurrencesWithPosition = sortedOccurrences.map(occurrence => (0, _extends2.default)({}, occurrence, {
      position: {
    ...
    return {
      occurrences: occurrencesWithPosition,
      maxIndex
    };
 ...
 */
function computeOccurrencesMaxIndex(adapter, occurrences) {
  const sortedOccurrences = (0, _sortEventOccurrences.sortEventOccurrences)(occurrences);
  const conflicts = buildOccurrenceConflicts(adapter, sortedOccurrences);
 ...
 */
function buildOccurrenceConflicts(adapter, occurrences) {
  const getEmptyBlock = () => ({
    occurrences: [],
    longestDurationMs: 0
  });
  const occurrencesBlocks = [];
  let currentBlock = getEmptyBlock();
  ...
  for (const block of occurrencesBlocks) {
    for (let i = 0; i < block.occurrences.length; i += 1) {
      ...
      const conflictsBefore = new Set();
      const conflictsAfter = new Set();
...
function buildFirstIndexLookup(conflicts) {
  let maxIndex = 1;
  const firstIndexLookup = {};
  for (const occurrence of conflicts) {
    if (occurrence.before.size === 0) {
      firstIndexLookup[occurrence.key] = 1;
    } else {
      const usedIndexes = new Set(Array.from(occurrence.before).map(conflictingOccurrence => firstIndexLookup[conflictingOccurrence]));
      let i = 1;
...
function buildLastIndexLookup(conflicts, firstIndexLookup, maxIndex, maxSpan) {
  if (maxSpan < 2) {
    return firstIndexLookup;
  }
  const lastIndexLookup = {};
  for (const occurrence of conflicts) {
    const usedIndexes = new Set([...Array.from(occurrence.before), ...Array.from(occurrence.after)].map(conflictingOccurrence => firstIndexLookup[conflictingOccurrence]));
    const firstIndex = firstIndexLookup[occurrence.key];
    let lastIndex = firstIndex;
    while (!usedIndexes.has(lastIndex + 1) && lastIndex < maxIndex && lastIndex - firstIndex < maxSpan - 1) {
      lastIndex += 1;
    }
