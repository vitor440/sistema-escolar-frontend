...
exports.dateToEventString = dateToEventString;
exports.getDateKey = getDateKey;
exports.getEndOfWeek = getEndOfWeek;
exports.getOccurrenceEnd = getOccurrenceEnd;
exports.getStartOfWeek = getStartOfWeek;
exports.getWallTimeIsoFormat = getWallTimeIsoFormat;
exports.getWeekNumber = getWeekNumber;
exports.mergeDateAndTime = mergeDateAndTime;
exports.normalizeAllDayBounds = normalizeAllDayBounds;
/**
 ...
 */
function getWallTimeIsoFormat(adapter) {
  const f = adapter.formats;
  const esc = adapter.escapedCharacters;
  return [f.yearPadded, esc.start, '-', esc.end, f.monthPadded, esc.start, '-', esc.end, f.dayOfMonthPadded, esc.start, 'T', esc.end, f.hours24hPadded, esc.start, ':', esc.end, f.minutesPadded, esc.start, ':', esc.end, f.secondsPadded].join('');
}
 ...
 */
function dateToEventString(adapter, date, originalString, dataTimezone) {
 ...
 */
function normalizeAllDayBounds(adapter, start, end, allDay) {
  if (!allDay) {
    return {
      start,
      end
 ...
 */
function getDateKey(day, adapter) {
 ...
 * adapter instance.
 */
const sundayDayNumberCache = new WeakMap();
function getSundayDayNumber(adapter) {
  ...
  if (cached === undefined) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (!Number.isInteger(cached) || cached < 1 || cached > 7) {
        ...
        throw new Error(`MUI X Scheduler: adapter.getDayOfWeek returned an unexpected value (${cached}).` + ' Expected an integer in the range 1–7.' + ' Check that your adapter implements getDayOfWeek correctly.');
      }
    }
 ...
 * is used unchanged.
 */
function getStartOfWeek(adapter, date, weekStartsOn) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!Number.isInteger(weekStartsOn) || weekStartsOn < 0 || weekStartsOn > 6) {
      // eslint-disable-next-line mui/no-guarded-throw
      throw new Error(`MUI X Scheduler: weekStartsOn must be an integer between 0 (Sunday) and 6 (Saturday),` + ` but received ${weekStartsOn}.`);
    }
  }
 ...
 */
function getEndOfWeek(adapter, date, weekStartsOn) {
  if (weekStartsOn === undefined) {
 ...
 */
function getWeekNumber(adapter, date, weekStartsOn) {
  if (weekStartsOn === undefined) {
