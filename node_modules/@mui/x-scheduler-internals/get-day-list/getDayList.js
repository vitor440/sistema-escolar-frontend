...
var _useAdapter = require("../use-adapter/useAdapter");
var _processDate = require("../process-date");
function getDayList(parameters) {
  const {
    adapter,
    start: rawStart,
    end: rawEnd,
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (adapter.isBefore(adapter.startOfDay(end), adapter.startOfDay(start))) {
      ...
      throw new Error(`MUI X Scheduler: getDayList: The 'end' parameter must be a day after the 'start' parameter.`);
    }
  }
  let current = start;
  ...
  while (adapter.isBefore(current, end)) {
    if (!excludeWeekends || !(0, _useAdapter.isWeekend)(adapter, current)) {
      days.push(current);
    }
  ...
  return days.map(day => (0, _processDate.processDate)(day, adapter));
}